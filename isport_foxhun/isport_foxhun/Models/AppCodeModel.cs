...
namespace isport_foxhun.Models
{
    public class AppCodeModel
    {
        ...
        {
            GMailer.GmailUsername = ConfigurationManager.AppSettings["Isport_Mail_To"];
            GMailer.GmailPassword = ConfigurationManager.AppSettings["Isport_Mail_password"];
            ...
            try
            {
                return SqlHelper.ExecuteNonQuery(strConnDB, CommandType.StoredProcedure, "usp_foxhun_playerinsert"
                    , new SqlParameter[] {
                        ...
                        ,new SqlParameter("@nameen",p.player.nameen)
                        ,new SqlParameter("@size",p.player.size)
                        ,new SqlParameter("@sizepants",p.player.sizepants)
                    });
            }
            catch(Exception ex)
            {
