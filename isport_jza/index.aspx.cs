    ...
    public partial class index : PageBase
    {
        ...
        public override void PreGenContent(string optCode, string projectType)
        {
            try
            {
                ...
                {
                    lblContent.Controls.Add(new isport_service.ServiceWapUI_Content().GenContent(AppCodeJza.strConn, AppCodeJza.strConnOracle, "", "0", mU.mobileOPT, Request["p"].ToString(), "index.aspx", "", ""));
                }
                else
                {
                    if (rtnXMl != null)
                    {
                        ...
                        IEnumerable<XElement> content = (from el in rtnXMl.Root.Elements("Content") where  (string)el.Attribute("type") == "jza" select el);
        ...
        {
            lblFooter.Controls.Add(new isport_service.ServiceWapUI_Header().GenHeader(AppCodeJza.strConn, optCode, "kiss", "", "index.aspx", Level.ToString()));
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!IsPostBack)
                {
                    ...
                    {
                        // service jza
                        DataSet ds = ServiceTded.GetSipContentByPcntId(AppCodeJza.strConnPack, Request["s"]);
